...
authors: "Script by Anna Walentowitz (2024), with adaptations by Franka Gaiser and Manuel Steinbauer (2025)"
date: 2025-09-12
format: pdf
editor: visual
---
...
library(here)       # helps to organize project structure
...
To organise your data it is helpful to create folders for your underlying data and the results of your analysis. Which you can do with the following simple code snippets and here() that automatically finds the path to your project:
...
Our source for climate data is [WorldClim](https://www.worldclim.org/), which you can access via the \``geodata`\` package. Be ware that the bioclimatic data from WorldClim is available at different spatial resolutions (10, 5, 2.5 and 0.5 minutes of a degree). The higher the resolution (lower values) the longer it takes to download and process the data.
...
#access from repo if download is too slow
#files <- list.files(here("data_predownloaded", 
#                                  "climate",
#                                  "wc2.1_5m"), 
#                    full.names = TRUE)
#bioclim_data2 <- terra::rast(files)
...
#access from repo if download is too slow
# gbif_ArnMon_download <- readr::read_rds(here("data_predownloaded",
#    "gbif_ArnMon_download.rds"))
...
# The above is a short version of this model formula, that could be changed based on your assumptions
#glm_ArnMon <- glm(pa ~ wc2.1_5m_bio_1+ wc2.1_5m_bio_2+wc2.1_5m_bio_3+wc2.1_5m_bio_4+ wc2.1_5m_bio_5+wc2.1_5m_bio_6+wc2.1_5m_bio_7+ wc2.1_5m_bio_8+wc2.1_5m_bio_9+wc2.1_5m_bio_10+ wc2.1_5m_bio_11+wc2.1_5m_bio_12+wc2.1_5m_bio_13+wc2.1_5m_bio_14+wc2.1_5m_bio_15+ wc2.1_5m_bio_16+wc2.1_5m_bio_17+wc2.1_5m_bio_18+ wc2.1_5m_bio_19, data = training_points[,-c(1,2)],                   family = binomial())
```
...
We take the GLM build before to predict the distribution of *Arnica montana* in 2061-2080 by exchanging the bioclimatic variables representing current climatic conditions by data based on climate models. We stick to the same resolution of 5 degrees.
...
#load from repo if download is too slow
# forecast_data <- readr::read_rds(here("data_prwdownloaded",
#  "gadm",
#  "gadm36_adm0_r2_pk.rds"))
...
# max_spec_sens is based on the sum of specificity (avoiding false positives) and sensitivity (catching presences).
...
```{r}
writeRaster(predict_ArnMon, here("results","ArnMon_current_distribution.tif"), overwrite = TRUE)
writeRaster(forecast_presence, here("results","ArnMon_future_distribution.tif"), overwrite = TRUE)
```
